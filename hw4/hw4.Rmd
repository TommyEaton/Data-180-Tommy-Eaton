...
author: 
  name: "Tommy Eaton"
  email: "eatot@dickinson.edu"
date: '`r format(Sys.Date(), "%B %d, %Y")`'
output: 
  html_document
---
# 16/20. Interpret your graphs. You are not answering the questions asked.
...
```{r}
summary_stat <- records %>%
  summarise_if(is.numeric,list(mean = ~mean(.), median = ~median(.), mode = ~mode(.)))
summary_stat
```
# Question 2
...
# count of cases where record was broken... not distinct records.. 
```
# Question 3
...
records %>%
  ...
  labs(title = "Distribution of Record Times by Track Type",
       x = "Record Time",
       y = "Count") +
  scale_fill_manual(values = c("Single" = "blue", "Three Lap" = "red")) +
  theme_minimal()
...
records %>%
  ggplot(aes(x = time_per, fill = type)) +  geom_histogram(alpha = 0.5, position = "identity", bins = 30) +
